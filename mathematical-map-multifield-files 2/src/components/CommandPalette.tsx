              ...
              >
                <Command className="glass overflow-hidden rounded-2xl shadow-2xl" loop>
                  ...
                  <Command.List className="max-h-[420px] overflow-y-auto p-2">
                    ...
                    <Command.Group heading="Maps" className="px-2 pt-2 text-[10px] uppercase tracking-widest text-white/40">
                      {Object.values(MAPS).map((map) => (
                        ...
                        >
                          <span className="flex w-full items-center justify-between gap-2">
                            <span className="text-white/85">{map.label}</span>
                            {map.id === mapId && <span className="text-[10px] text-accent-cyan">current</span>}
                          </span>
                        </Item>
                      ))}
                    </Command.Group>

                    <Command.Group heading="Actions" className="px-2 pt-3 text-[10px] uppercase tracking-widest text-white/40">
                    ...
                    <Command.Group heading="Nodes" className="px-2 pt-3 text-[10px] uppercase tracking-widest text-white/40">
                      {data.nodes.map((n) => (
                        <Item
                          key={n.id}
                        ...
                        >
                          <span className={cn(`kind-${n.kind}`, "flex w-full items-center gap-2")}>
                            ...
                            <span className="w-[110px] text-[11px] text-white/40">{KIND_LABEL[n.kind]}</span>
                            <span className="truncate text-[13px] text-white/90">{n.title}</span>
                            <span className="ml-auto max-w-[140px] truncate text-[10px] text-white/30">{n.topicCluster}</span>
                          </span>
                        </Item>
                      ))}
                    </Command.Group>
                    ...
                    {showPath && (
                      <Command.Group heading="Learning Path" className="px-2 pt-3 text-[10px] uppercase tracking-widest text-white/40">
                        {data.nodes.map((n) => (
